    ...
    public boolean pcal_fiducial_cut(int current_Part, HipoDataBank rec_Bank, HipoDataBank cal_Bank) {
        for (int current_Row = 0; current_Row < cal_Bank.rows(); current_Row++) {
            if (cal_Bank.getInt("pindex", current_Row)==current_Part && 
                ...
                if (pid == 22) {
                    return lv > 15 && lw > 15; // medium cut for photons (Stefan using 15, Chris using 14?)
                } else {
...
//        return true;
    ...
    public boolean calorimeter_sampling_fraction_cut(int current_Part, double p, HipoDataBank run_Bank, 
...
//        if (runnum < 10000) {
            ...
            double e_cal_sampl_mu[][] = {{0.2520, 0.2520, 0.2479, 0.2444, 0.2463, 0.2478},
                {-0.8615, -0.8524, -0.6848, -0.5521, -0.5775, -0.7327},
                {5.596, 6.522, 5.752, 5.278, 6.430, 5.795}};
        
            double e_cal_sampl_sigma[][] = {{-0.02963, -0.1058, -0.05087, -0.04524, -0.02951, -0.01769}, 
        ...
        // RGB winter 2020 // (also using this for RGB fall 2019, but it should be updated! TODO)
            double e_cal_sampl_mu[][] = {{0.2433, 0.2421, 0.2415, 0.2486, 0.2419, 0.2447},
                {-0.8052, -1.0495, -1.1747, -0.5170, -0.6840, -0.9022},
                {5.2750, 4.4886, 4.4935, 5.9044, 5.6716, 4.9288}};
        
            double e_cal_sampl_sigma[][] = {{0.0120, 0.0164, 0.0120, 0.0108, 0.0147, 0.0077}, 
...
//        return Math.abs(vz) < 13;
    ...
    public boolean random_photon_cut(int current_Part, HipoDataBank rec_Bank) {
    ...
    public boolean electron_test(int current_Part, double p, float vz, HipoDataBank rec_Bank, HipoDataBank cal_Bank, 
        ...
        return true
            ...
            && calorimeter_sampling_fraction_cut(current_Part, p, run_Bank, cal_Bank)
...
//            && p > 0.4
    ...
    public boolean photon_test(int current_Part, HipoDataBank rec_Bank, HipoDataBank cal_Bank, 
        ...
        return true
            ...
            && forward_detector_cut(current_Part, rec_Bank)
            && pcal_fiducial_cut(current_Part, rec_Bank, cal_Bank)
            && random_photon_cut(current_Part, rec_Bank)
...
//                if (pid == -321) { pid = 211; }
            ...
            for (int current_Part = 0; current_Part < rec_Bank.rows(); current_Part++) {
                ...
                if (particle_test(current_Part, rec_Bank) 
                    && photon_test(current_Part, rec_Bank, cal_Bank, lv_e, lv_gamma)) {
