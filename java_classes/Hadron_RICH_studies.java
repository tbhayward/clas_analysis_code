...
public class Hadron_RICH_studies {
    ...
    protected int RICH_pid; // old variables
    protected int hindex, pindex, emilay, emico, t, emqua, best_PID; // additional pass-2 variables
    protected double chi2pid, beta, RQ_prob, el_prob, pi_prob, k_prob, pr_prob; // old variables
    protected double mchi2, ReQ_prob, best_etaC, best_c2, best_RL, best_ntot, best_mass; // additional pass-2 variables
...
//        else if (variables.Mx()<1.5) { return false; } 
    ...
    public Hadron_RICH_studies(DataEvent event, PhysicsEvent recEvent, int pPID, int pIndex) {
...
//            if (rich_Bank.rows() <= 1) {
                pindex = rich_Bank.getInt("pindex", 0);
                ...
                phi = phi - 90; if (phi < 0) { phi = 360 + phi; } phi = 360 - phi;
                phi = Math.toRadians(phi);
                if ( Math.toDegrees(theta-p_theta)<2 && Math.toDegrees(phi-p_phi)<6) {
                    hindex = rich_Bank.getInt("hindex", 0);
                    emilay = rich_Bank.getInt("emilay", 0);
                    emico = rich_Bank.getInt("emico", 0);
                    t = rich_Bank.getInt("t", 0);
                    emqua = rich_Bank.getInt("emqua", 0);
                    best_PID = rich_Bank.getInt("best_PID", 0);
                    
                    mchi2 = rich_Bank.getFloat("mchi2", 0);
                    ReQ_prob = rich_Bank.getFloat("RQ_prob", 0);
                    ...
                    best_etaC = rich_Bank.getFloat("best_etaC", 0);
                    best_c2 = rich_Bank.getFloat("best_c2", 0);
                    best_RL = rich_Bank.getFloat("best_RL", 0);
                    best_ntot = rich_Bank.getFloat("best_ntot", 0);
                    best_mass = rich_Bank.getFloat("best_mass", 0);
                }
//            }
        }
    ...
    public int pindex() { return pindex; } // returns related row in the REC::Particle bank
    
    public int hindex() { return hindex; } // returns related row in the RICH::clusters bank (if any)
    
    public int emilay() { return emilay; } // returns aerogel layer of photon emission
    
    public int emico() { return emico; } // returns aerogel component of photon emission
    
    public int t() { return t; } // returns aerogel component of particle entrance point
    
    public int emqua() { return emqua; } // returns emqua
    
    public double mchi2() { return mchi2; } // returns most probable PID choice
    
    public int best_PID() { return best_PID; } // returns aerogel layer of photon emission
    ...
    public double RQ_prob() { return Double.valueOf(Math.round(RQ_prob*100000))/100000; }// goodness of hadron 
        // choice parameter (1=anambiguos, 0=random)
    ...
    public double best_etaC() { return Double.valueOf(Math.round(best_etaC*100000))/100000; }// returns Average 
        // etaC for best hypothesis
    
    public double best_c2() { return Double.valueOf(Math.round(best_c2*100000))/100000; }// returns chi2 for 
        // best hypothesis
    
    public double best_RL() { return Double.valueOf(Math.round(best_RL*100000))/100000; }// returns Likelihood ratio 
        // for best hypothesis
    
    public double best_ntot() { return Double.valueOf(Math.round(best_ntot*100000))/100000; }// returns Number of 
        // photon used for likelihood
    
    public double best_mass() { return Double.valueOf(Math.round(best_mass*100000))/100000; }// returns reconstructed 
        // mass for best thesis
